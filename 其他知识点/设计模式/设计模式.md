...
# 二 GoF 的 23 种设计模式的分类和功能

设计模式有两种分类方法，即根据`模式的目的`来分和根据`模式的作用的范围`来分。根据模式是用来完成什么工作来划分，这种方式可分为、式和 3 种。
...
本教程主要介绍软件设计模式)中经常用到的类图，以及类之间的关系。另外，在实验部分将简单介绍 UML 建模工具的使用方法，当前业界使用最广泛的是 Rational Rose。使用 Umlet 的人也很多，它是一个轻量级的开源 UML 建模工具，简单实用，常用于小型软件系统的开发与设计。
...
![Student ç±»](mdpic/3-1Q1121K933555.gif)
...
![âè®¡ç®é¿æ¹å½¢ååå½¢çå¨é¿ä¸é¢ç§¯âçç±»å¾](mdpic/3-1Q1121P6195T.gif)
...
![ä¾èµå³ç³»çå®ä¾](mdpic/3-1Q1121PA2Y5.gif)
...
![关联关系的实例](mdpic/3-1Q1121Q5115Q.gif)
																				图5 关联关系的实例
...
![聚合关系的实例](mdpic/3-1Q1121Q541410.gif)
																				图6 聚合关系的实例
...
![组合关系的实例](mdpic/3-1Q1121QFD27.gif)
图7 组合关系的实例
...
![æ³åå³ç³»çå®ä¾](mdpic/3-1Q1121Q62C57.gif)
...
![实现关系的实例](mdpic/3-1Q1121QI4317.gif)
								图9 实现关系的实例
...
![Windows的桌面主题类图](mdpic/3-1Q113100151L5.gif)
						图1 Windows的桌面主题类图
...
![âå ç»´é¸ä¸æ¯é¸âå®ä¾çç±»å¾](mdpic/3-1Q11311094H32.gif)
...
![“几维鸟是动物”实例的类图](mdpic/3-1Q11311101SN.gif)
						图2 “几维鸟是动物”实例的类图
...
![顾客购物程序的类图](mdpic/3-1Q113131610L7.gif)
																					图1 顾客购物程序的类图
...
![大学学生工作管理程序的类图](mdpic/3-1Q113133F4161.gif)
图1 大学学生工作管理程序的类图
...
![å­¦çæç»©ç®¡çç¨åºçç±»å¾](mdpic/3-1Q113141Q0X3.gif)
...
![ææä¸ç»çºªäººçå³ç³»å¾](mdpic/3-1Q113152Q5W1.gif)
...
![ç¨ç»§æ¿å³ç³»å®ç°çæ±½è½¦åç±»çç±»å¾](mdpic/3-1Q113160133151.gif)
...
![用组合关系实现的汽车分类的类图](mdpic/3-1Q11316034X57.gif)
															图2 用组合关系实现的汽车分类的类图

以上共 7 种设计原则，它们分别为`开闭原则`、`里氏替换原则`、`依赖倒置原则`、`单一职责原则`、`接口隔离原则`、`迪米特法则`和本`合成复用原则`。
...
创建型模式的主要关注点是“怎样创建对象？”，它这样可以降低系统的耦合度，使用者不需要关注对象的创建细节，对象的创建由相关的工厂来完成。就像我们去商场购买商品时，不需要知道商品是怎么生产出来一样，因为它们由专门的厂商生产。
...
以上 5 种创建型模式，除了`工厂方法`模式属于类创建型模式，其他的全部属于对象创建型模式
...
![单例模式的结构图](mdpic/3-1Q1131K441K2.gif)
																	图1 单例模式的结构图
...
![美国总统生成器的结构图](mdpic/3-1Q1131K529345.gif)
										图2 美国总统生成器的结构图
...
![çªå«æçæå¨çç»æå¾](mdpic/3-1Q1131K55X41.gif)
...
![猪八戒生成器的运行结果](mdpic/3-1Q1131I0563U.gif)
																				图4 猪八戒生成器的运行结果
...
![æéçå¤ä¾æ¨¡å¼çç»æå¾](mdpic/3-1Q1131KQ4K8.gif)
...
![原型模式的结构图](mdpic/3-1Q114101Fa22-20190729231119986.gif)
																			图1 原型模式的结构图
...
型模式的克隆分为浅克隆和深克隆，Java 中的 Object 类提供了浅克隆的 clone() 方法，具体原型类只要实现 Cloneable 接口就可实现对象的浅克隆，这里的 Cloneable 接口就是抽象原型类。其代码如下：
...
![孙悟空生成器的结构图](mdpic/3-1Q114101K4L9.gif)
																			图2 孙悟空生成器的结构图
...
![孙悟空克隆器的运行结果](mdpic/3-1Q114102002601.gif)
																						图3 孙悟空克隆器的运行结果
...
![奖状生成器的结构图](mdpic/3-1Q114101SUJ.gif)
																图4 奖状生成器的结构图
...
public class ProtoTypeCitation
{
    public static void main(String[] args) throws CloneNotSupportedException
    {
        citation obj1=new citation("张三","同学：在2019学年第一学期中表现优秀，被评为三好学生。","韶关学院");
        obj1.display();
...
```
奖状创建成功！
张三同学：在2019学年第一学期中表现优秀，被评为三好学生。韶关学院
奖状拷贝成功！
李四同学：在2019学年第一学期中表现优秀，被评为三好学生。韶关学院
```
...
![å¸¦ååç®¡çå¨çååæ¨¡å¼çç»æå¾](mdpic/3-1Q114102049214.gif)
...
![图形生成器的结构图](mdpic/3-1Q11410212a20.gif)
																							图6 图形生成器的结构图
...
![工厂方法模式的结构图](mdpic/3-1Q114135A2M3.gif)
																									图1 工厂方法模式的结构图
...
![畜牧场结构图](mdpic/3-1Q11413554DT.gif)
																												图2 畜牧场结构图
...
![畜牧场养殖的运行结果](mdpic/3-1Q114135422T9.gif)
																	图3 畜牧场养殖的运行结果
...
![简单工厂模式的结构图](mdpic/3-1Q114135306120.gif)
													图4 简单工厂模式的结构图
...
![电器工厂的产品等级与产品族](mdpic/3-1Q1141559151S.gif)
...
![æ½è±¡å·¥åæ¨¡å¼çç»æå¾](mdpic/3-1Q11416002NW.gif)
...
![ååºç±»çç»æå¾](mdpic/3-1Q114160132648.gif)
...
![农场养殖的运行结果](mdpic/3-1Q11416021C61.jpg)
															图4 农场养殖的运行结果
...
![建造者模式的结构图](mdpic/3-1Q1141H441X4-20190729233213664.gif)
																					图1 建造者模式的结构图
...
![客厅装修的结构图](mdpic/3-1Q1141H553419.gif)
																		图2 客厅装修的结构图
...
![客厅装修的运行结果](mdpic/3-1Q1141HK9195.jpg)
																						图3 客厅装修的运行结果
...
以上 7 种结构型模式，除了`适配器模式`分为`类结构型模式`和`对象结构型模式`两种，其他的全部属于对象结构型模式，
...
![代理模式的结构图](mdpic/3-1Q115093011523.gif)
																						图1 代理模式的结构图
...
![韶关“天街e角”公园的结构图](mdpic/3-1Q115093110520.gif)
															图2 韶关“天街e角”公司的结构图
...
![é¶å³âå¤©è¡eè§âå¬å¸çä»£çäº§å](mdpic/3-1Q1150932145Y.jpg)
...
![å¨æä»£çæ¨¡å¼çç»æå¾](mdpic/3-1Q115093255227.gif)
...
类适配器模式可采用多重继承方式实现，如 [C++](http://c.biancheng.net/cplus/) 可定义一个适配器类来同时继承当前系统的业务接口和现有组件库中已经存在的组件接口；Java不支持多继承，但可以定义一个适配器类来实现当前系统的业务接口，同时又继承现有组件库中已经存在的组件。
...
![类适配器模式的结构图](mdpic/3-1Q1151045351c.gif)
																图1 类适配器模式的结构图

对象适配器模式的结构图如图 2 所示。

![对象适配器模式的结构图](mdpic/3-1Q1151046105A.gif)
													图2 对象适配器模式的结构图
...
![发动机适配器的结构图](mdpic/3-1Q115104I22F.gif)
															图3 发动机适配器的结构图
...
![双向适配器模式的结构图](mdpic/3-1Q115104Q1604.gif)
											图4 双向适配器模式的结构图
...
![桥接模式的结构图](mdpic/3-1Q115125253H1.gif)
																					图1 桥接模式的结构图
...
![女士皮包选购的结构图](mdpic/3-1Q11512532X54.gif)
																图2 女士皮包选购的结构图
...
![女士皮包选购的运行结果1](mdpic/3-1Q115125404105.jpg)
											图3 女士皮包选购的运行结果1
...
![女士皮包选购的运行结果2](mdpic/3-1Q11512542a20.jpg)
					图4 女士皮包选购的运行结果2
...
![桥接模式与适配器模式联用的结构图](mdpic/3-1Q1151255004c.gif)
图5 桥接模式与适配器模式联用的结构图
...
![装饰模式的结构图](mdpic/3-1Q115142115M2.gif)
													图1 装饰模式的结构图
...
​																![游戏角色“莫莉卡·安斯兰”的结构图](mdpic/3-1Q115142204235.gif)
...
![游戏角色“莫莉卡·安斯兰”的变身](mdpic/3-1Q115142234201.gif)
图3 游戏角色“莫莉卡·安斯兰”的变身
...
装饰模式在  语言中的最著名的应用莫过于 Java I/O 标准库的设计了。例如，InputStream 的子类 FilterInputStream，OutputStream 的子类 FilterOutputStream，Reader 的子类 BufferedReader 以及 FilterReader，还有 Writer 的子类 BufferedWriter、FilterWriter 以及 PrintWriter 等，它们都是抽象装饰类。
...
![只有一个具体构件的装饰模式](mdpic/3-1Q11514230H05-20190730222951548.gif)
图4 只有一个具体构件的装饰模式
...
![只有一个具体装饰的装饰模式](mdpic/3-1Q115142333D3.gif)
...
![åçæ¿äº§è¯è¿æ·çç¸å³é¨é¨](mdpic/3-1Q11515205E60.gif)
...
![å¤è§æ¨¡å¼çç»æå¾](mdpic/3-1Q115152143509.gif)
...
![婺源特产管理界面的结构图](mdpic/3-1Q115152223406.gif)
														图3 婺源特产管理界面的结构图
...
![婺源特产管理界面的运行结果](mdpic/3-1Q1151523101b.jpg)
图4 婺源特产管理界面的运行结果
...
​						![引入抽象外观类的外观模式的结构图](mdpic/3-1Q1151524262a.gif)
​																			图5 引入抽象外观类的外观模式的结构图
...
![享元模式的结构图](mdpic/3-1Q115161342242.gif)
																				图1 享元模式的结构图
...
![五子棋游戏的结构图](mdpic/3-1Q11516141M29.gif)
																			图2 五子棋游戏的结构图
...
![五子棋游戏的运行结果](mdpic/3-1Q115162I4425.gif)
图3 五子棋游戏的运行结果
...
(1) 单纯享元模式，这种享元模式中的所有的具体享元类都是可以共享的，不存在非共享的具体享元类，(1)其结构图如图 4 所示。



![单纯享元模式的结构图](mdpic/3-1Q115161549429.gif)
													图4 单纯享元模式的结构图
...
![复合享元模式的结构图](mdpic/3-1Q11516162C42.gif)
																						图5 复合享元模式的结构图
...
组合（Composite）模式的定义：有时又叫作，它是一种将对象组合成树状的层次结构的模式，用来表示“部分-整体”的关系，使用户对单个对象和组合对象具有一致的访问性。
...
​											![透明式的组合模式的结构图](mdpic/3-1Q1151G62L17.gif)
​																							图1 透明式的组合模式的结构图
...
![安全式的组合模式的结构图](mdpic/3-1Q1151GF5221.gif)
											图2 安全式的组合模式的结构图
...
![集合c0的树状图](mdpic/3-1Q1151GUH32.gif)
图3 集合c0的树状图
...
![韶关“天街e角”店购物的结构图](mdpic/3-1Q1151G924356.gif)
									图4 韶关“天街e角”店购物的结构图
...
如果对前面介绍的组合模式中的树叶节点和树枝节点进行抽象，也就是说树叶节点和树枝节点还有子节点，这时组合模式就扩展成复杂的组合模式了，如 Java AWT/[Swing中的简单组件 JTextComponent 有子类 JTextField、JTextArea，容器组件 Container 也有子类 Window、Panel。复杂的组合模式的结构图如图 5 所示。

![复杂的组合模式的结构图](mdpic/3-1Q1151H632613.gif)
图5 复杂的组合模式的结构图
...
以上 11 种行为型模式，除了模板方法模式和是类行为型模式，其他的全部属于对象行为型模式，下面我们将详细介绍它们的特点、结构与应用。
...
​										![模板方法模式的结构图](mdpic/3-1Q116095405308.gif)
​																										图1 模板方法模式的结构图
...
​														![出国留学手续设计程序的结构图](mdpic/3-1Q11609544UV.gif)
​																							图2 出国留学手续设计程序的结构图
...
![含钩子方法的模板方法模式的结构图](mdpic/3-1Q116095550123.gif)
												图3 含钩子方法的模板方法模式的结构图
...
![策略模式的结构图](mdpic/3-1Q116103K1205.gif)
							图1 策略模式的结构图
...
![大闸蟹做菜策略的结构图](mdpic/3-1Q116103R5638.gif)
														图2 大闸蟹做菜策略的结构图
...
![å¤§é¸è¹åèç»æ](mdpic/3-1Q116103Z1315.jpg)
...
![婺源旅游结构图](mdpic/3-1Q11610393E92.gif)
																	图4 婺源旅游结构图
...
![策略工厂模式的结构图](mdpic/3-1Q116104010550.gif)
图5 策略工厂模式的结构图
...
![命令模式的结构图](mdpic/3-1Q11611335E44.gif)
图1 命令模式的结构图
...
![客户在餐馆吃早餐的结构图](mdpic/3-1Q1161134341E.gif)
图2 客户在餐馆吃早餐的结构图
...
![客户在餐馆吃早餐的运行结果](mdpic/3-1Q11611350Q06.jpg)
图2 客户在餐馆吃早餐的运行结果
...
![组合命令模式的结构图](mdpic/3-1Q1161135322R.gif)
图3 组合命令模式的结构图
...
![责任链模式的结构图](mdpic/3-1Q116135Z11C.gif)
图1 责任链模式的结构图



![责任链](mdpic/3-1Q11613592TF.gif)
图2 责任链
...
![请假条审批模块的结构图](mdpic/3-1Q11614000IV.gif)
...
![状态模式的结构图](mdpic/3-1Q11615412U55.gif)
...
![学生成绩的状态转换程序的结构图](mdpic/3-1Q11615425V39.gif)
图2 学生成绩的状态转换程序的结构图
...
![线程状态转换图](mdpic/3-1Q116154332451.gif)
图3 线程状态转换图
...
![线程状态转换程序的结构图](mdpic/3-1Q116154432352.gif)
图4 线程状态转换程序的结构图
...
![共享状态模式的结构图](mdpic/3-1Q116154539147.gif)
图5 共享状态模式的结构图
...
![观察者模式的结构图](mdpic/3-1Q1161A6221S.gif)
图1 观察者模式的结构图
...
![人民币汇率分析程序的结构图](mdpic/3-1Q1161A646395.gif)
图2 人民币汇率分析程序的结构图
...
![学校铃声的事件模型图](mdpic/3-1Q1161AGQ46.gif)
图3 学校铃声的事件模型图
...
![学校铃声事件处理程序的结构图](mdpic/3-1Q1161AP0K8.gif)
图4 学校铃声事件处理程序的结构图
...
![原油期货的观察者模式实例的结构图](mdpic/3-1Q1161ARKO.gif)
图5 原油期货的观察者模式实例的结构图
...
![ä¸­ä»èæ¨¡å¼çç»æå¾](mdpic/3-1Q1161I532V0.gif)
图1 中介者模式的结构图
...
![韶关房地产交流平台的结构图](mdpic/3-1Q1161I559410.gif)
图2 韶关房地产交流平台的结构图
...
![韶关房地产交流平台的运行结果](mdpic/3-1Q1161I6313F.gif)
图3 韶关房地产交流平台的运行结果
...
![ç®åä¸­ä»èæ¨¡å¼çç»æå¾](mdpic/3-1Q1161IA5242.gif)
...
![迭代器模式的结构图](mdpic/3-1Q1161PU9528.gif)
图1 迭代器模式的结构图
...
![å©ºæºææ¸¸é£æ¯å¾æµè§ç¨åºçç»æå¾](mdpic/3-1Q1161P9263W.gif)
...
![婺源旅游风景图浏览程序的运行结果](mdpic/3-1Q1161Q029442.jpg)
图3 婺源旅游风景图浏览程序的运行结果
...
迭代器模式常常与结合起来使用，在对组合模式中的容器构件进行访问时，经常将迭代器潜藏在组合模式的容器构成类中。当然，也可以构造一个外部迭代器来对容器构件进行访问，其结构图如图 4 所示。

![ç»åè¿­ä»£å¨æ¨¡å¼çç»æå¾](mdpic/3-1Q1161Q152626.gif)
...
![艺术公司设计的产品](mdpic/3-1Q119101U2436.jpg)
(a)艺术公司设计的产品



![造币公司生产的货币](mdpic/3-1Q119101921H6.jpg)
(b)造币公司生产的货币
...
![包含组合模式的访问者模式的结构图](mdpic/3-1Q11910210Jc.gif)
图4 包含组合模式的访问者模式的结构图
...
![备忘录模式的结构图](mdpic/3-1Q119130413927.gif)
图1 备忘录模式的结构图
...
![相亲游戏的结构图](mdpic/3-1Q119130439230.gif)
图2 相亲游戏的结构图
...
[![相亲游戏的运行结果](mdpic/3-1Q119130526391.jpg)](mdpic/3-1Q1191305555M.jpg)
图3 相亲游戏的运行结果
...
![带原型的备忘录模式的结构图](mdpic/3-1Q119130HW56.gif)
图4 带原型的备忘录模式的结构图
...
![å¥å­âææ¯å¤§å­¦çâçè¯­æ³æ ](mdpic/3-1Q119150550114.gif)
...
![解释器模式的结构图](mdpic/3-1Q119150626422.gif)
图2 解释器模式的结构图
...
![“韶粵通”公交车读卡器程序的结构图](mdpic/3-1Q119150Q6401.gif)
图3 “韶粵通”公交车读卡器程序的结构图
